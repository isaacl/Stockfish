...
#ifdef CRAZYHOUSE
  if (is_drop(m) && (!pieceCountInHand[us][type_of(pc)] || !empty(to)))
      return false;
  else
#endif
  if (pieces(us) & to)
...
#ifdef CRAZYHOUSE
  if (st->checkSquares[type_of(is_drop(m) ? dropped_piece(m) : piece_on(from))] & to)
#else
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  Square to = to_sq(m);
#ifdef CRAZYHOUSE
  Piece pc = is_drop(m) ? dropped_piece(m) : piece_on(from);
#else
...
#ifdef CRAZYHOUSE
  assert(is_drop(m) || empty(from) || type_of(m) == CASTLING);
#else
...
#ifdef CRAZYHOUSE
  PieceType nextVictim = type_of(is_drop(m) ? dropped_piece(m) : piece_on(from));
  Color stm = ~color_of(is_drop(m) ? dropped_piece(m) : piece_on(from)); // First consider opponent's move
#else
